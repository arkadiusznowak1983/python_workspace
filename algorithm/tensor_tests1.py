...
class TensorLayer(object):
    ...
    def __init__(self, inputs_count, nodes):
        ...
        self.layer['biasy'] = tf.Variable(tf.zeros([nodes])) if self.ustawienia('WYZEROWANY_BIAS') else tf.Variable(
            tf.random_normal([nodes]))
...
class TensorNetwork(object):
    ...
    @property
    def jakCzestoBlad(self):
        return self._JAK_CZESTO_POKAZAC_BLAD

    @jakCzestoBlad.setter
    def jakCzestoBlad(self, value):
        self._JAK_CZESTO_POKAZAC_BLAD = value

    @layers.setter
    def layers(self, value):
        return self._layers.append(value)
